...
export async function sendBookingRequestEmails(
  ...
  const customerAlready = await wasEmailAlreadySent({
    emailType: "booking.request.customer",
    bookingId: payload.bookingId,
    recipientEmail: payload.customerEmail,
  });
  const businessAlready = await wasEmailAlreadySent({
    emailType: "booking.request.business",
    bookingId: payload.bookingId,
    recipientEmail: payload.notificationEmail,
  });

  if (customerAlready && businessAlready) {
    return { customerSent: true, businessSent: true };
  }
  ...
  let customerSent = customerAlready;
  if (!customerAlready) {
  ...
  let businessSent = businessAlready;
  if (!businessAlready) {
